...
const botSwaps = {
  ...
  "Factory": { //no way to distinguish between day and night using name so be careful
    //"bosssanitar" : "sectantpriest" : "bosstsar"
  },
  "Interchange": {
...
class BotsController {
  ...
  static generateBot(bot, role, pmcData) {
   ...
   //else if (loweredRole == "bosstsar") {
   //  bot.Info.Side = "Boss"
   //  node = global._database.bots["bosstsar"];
   //  bot.Info.Voice = "Usec_1";
   //}
    else {
    ...
    if(loweredRole.includes("knight") || loweredRole.includes("bigpipe") || loweredRole.includes("tsar") || loweredRole.includes("birdeye")) {
      console.log(bot);
    }
